...
def validate_patient_upload_type(
    ...
    expected, signature_check = allowed[ext]
...
def patient_status(status: str) -> str:
    return PATIENT_STATUS_MAP.get(status, "processing")
    ...
    return (
        await db.execute(
            ...
            .join(
                ...
                PatientExternalRecordImport.source_document_id == DocumentStorageRecord.id,
            )
            .where(
                PatientExternalRecordImport.patient_id == patient_id,
...
async def stage_patient_external_record(
    ...
    *,
    ...
    if category is None:
        raise HTTPException(status_code=422, detail={"error_code": "UNSUPPORTED_RECORD_CATEGORY"})
    ...
    try:
        stored = await storage.put_patient_document(
            data, patient_id=patient_id, mime_type=mime_type
        )
    ...
    duplicate = await _find_import_by_hash(
        db, patient_id=patient_uuid, content_hash=stored.content_hash
    )
    if duplicate is not None:
        await storage.delete_patient_document(stored.storage_ref, patient_id=patient_id)
    ...
    try:
        ...
        await enqueue_audit_event(
            ...
            idempotency_key=f"patient-external-record-upload:{request_id}",
        ...
        await db.rollback()
        await storage.delete_patient_document(stored.storage_ref, patient_id=patient_id)
        replay = await _find_import_by_request(
            db, patient_id=patient_uuid, request_id=request_id
        ) or await _find_import_by_hash(
            db, patient_id=patient_uuid, content_hash=stored.content_hash
        )
        if replay is not None:
            return PatientImportUploadResult(replay, True)
        raise HTTPException(
            status_code=409,
            detail={"error_code": "IMPORT_IDEMPOTENCY_CONFLICT", "retryable": True},
        ) from None
    except Exception:
        await db.rollback()
        await storage.delete_patient_document(stored.storage_ref, patient_id=patient_id)
...
async def get_patient_external_record(
    ...
    if row is None:
        raise HTTPException(status_code=404, detail={"error_code": "EXTERNAL_RECORD_NOT_FOUND"})
    return row
...
async def read_patient_external_record_source(
    ...
    row = await get_patient_external_record(db, patient_id=patient_id, import_id=import_id)
    document = (
        await db.execute(
    ...
    if document is None:
        raise HTTPException(status_code=404, detail={"error_code": "SOURCE_DOCUMENT_NOT_FOUND"})
    try:
        data = await get_document_storage().get_patient_document_bytes(
            document.storage_ref, patient_id=patient_id
        )
    except DocumentStorageError as exc:
        raise HTTPException(
            status_code=503,
