...
from app.services.provider_auth_service import mark_provider_session_mfa_verified, verify_totp_code_once
...
async def verify_action_mfa(
    ...
    if not secret or not await verify_totp_code_once(provider.provider.provider_id, secret, payload.code, redis_client=get_async_redis_client()):
