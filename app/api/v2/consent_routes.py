...
Exposes consent challenge and break-glass endpoints. The legacy generic
`/grant` and `/routine/issue` endpoints are retained only as retired routes;
they cannot parse a patient identity or issue a routine capability.
"""
...
_break_glass_limiter = RateLimiter(
    max_requests=3, window_seconds=3600, key_func=lambda r: "bg"
)
...
class RoutineConsentGrantRequest(BaseModel):
    ...
    scope: list[str] = Field(
        ..., min_length=1, description="List of required namespaced data scopes"
    )
    assurance_level: AssuranceLevel = AssuranceLevel.STANDARD
    assurance_evidence: dict = Field(default_factory=dict)
...
def _token_hash(token: str) -> str:
    ...
    clean = (
        token[len("nexa:consent:") :] if token.startswith("nexa:consent:") else token
    )
...
async def grant_consent_route(
    ...
    """Retired: provider UUID input cannot authorize routine access."""
    raise HTTPException(
        status_code=status.HTTP_410_GONE,
        detail={"error_code": "ROUTINE_DIRECT_ISSUANCE_RETIRED"},
    )


@router.post("/routine/issue")
async def issue_routine_consent_route(
    provider: ProviderContext = Depends(get_provider_context),
):
    """Retired: routine consent follows a discovery-bound signed approval."""
    raise HTTPException(
        status_code=status.HTTP_410_GONE,
        detail={"error_code": "ROUTINE_DIRECT_ISSUANCE_RETIRED"},
    )
...
async def _deliver_consent_notification(
    *,
    ...
    await _redis_call(
        ...
        key,
        json.dumps(data),
        ex=ttl if isinstance(ttl, int) and ttl > 0 else 120,
    )
...
_PROMOTE_CONSENT_REQUEST_LUA = """
local raw = redis.call('GET', KEYS[1])
if not raw then return 0 end
local data = cjson.decode(raw)
if data.status ~= 'pending_audit' then return 0 end
...
async def _resolve_signed_approval_atomic(
    redis, request_id: str, nonce: str, data: dict, ttl: int
) -> bool:
...
async def _promote_consent_request_atomic(redis, request_id: str) -> bool:
    """Activate an audited challenge without extending its capability lifetime."""
    result = await _redis_call(
        redis.eval,
        _PROMOTE_CONSENT_REQUEST_LUA,
        1,
...
@router.post(
    ...
    response_model=ConsentChallengeResponsePayload,
)
async def create_consent_request(
    ...
    SECURITY: provider and patient identity are derived from the authenticated
    session and the one-time discovery handle. Client identity fields are not
    accepted as authorization anchors.
    """
    ...
    access_duration = max(
        MIN_DURATION, min(MAX_DURATION, payload.access_duration_seconds)
    )
    ...
    challenge_payload = {
        ...
        # This record is deliberately inert until the mandatory creation audit
        # has succeeded and the atomic promotion below has won.
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail={
                "error_code": "CONSENT_SECURITY_STORE_UNAVAILABLE",
                "retryable": True,
            },
        ) from exc
    ...
    try:
        ...
        raise HTTPException(
            status_code=503,
            detail={
                "error_code": "CONSENT_SECURITY_AUDIT_UNAVAILABLE",
                "retryable": True,
            },
        ) from exc

    try:
        promoted = await _promote_consent_request_atomic(redis, request_id)
    except Exception:
        promoted = False
    if not promoted:
        # Deletion is hygiene only. A failed deletion leaves pending_audit,
        # which every reader treats as unavailable and non-actionable.
        try:
            await _redis_call(redis.delete, f"consent_request:{request_id}")
        except Exception:
            pass
...
@router.post(
    ...
    response_model=SignedApprovalResponsePayload,
)
async def approve_signed_consent(
    ...
    if (
        await _redis_call(redis.get, f"biometric_nonce:{payload.challenge_nonce}:used")
    ...
    if payload.decision == "approved":
        ...
        if not await _resolve_signed_approval_atomic(
            ...
            payload.challenge_nonce,
            data,
            int(data.get("access_duration", 900)),
        ):
            raise HTTPException(
                status_code=409, detail={"error_code": "CONSENT_REPLAY_REJECTED"}
            )
        try:
            await push_request_limiter.release(patient_id=patient_id)
        except Exception as exc:
            logger.error(
                "Resolved consent push lock cleanup failed",
    ...
    data["approval_fingerprint"] = approval_fingerprint
    if not await _resolve_signed_approval_atomic(
        redis, payload.request_id, payload.challenge_nonce, data, 300
    ):
        raise HTTPException(
            status_code=409, detail={"error_code": "CONSENT_REPLAY_REJECTED"}
        )
    try:
        ...
        logger.error(
            "Denied consent push lock cleanup failed",
            extra={"error_type": type(exc).__name__},
        )
...
@router.get(
    "/status/{request_id}",
    status_code=status.HTTP_200_OK,
    response_model=ConsentStatusResponsePayload,
)
async def get_consent_request_status(
    ...
    if not raw:
        return ConsentStatusResponsePayload(
            request_id=request_id, status="expired", responded_at=None
        )
    ...
    if data.get("status") not in live_statuses:
        # Never expose an unaudited or unknown lifecycle state as a
        # provider-visible consent request.
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def claim_approved_access(
    ...
    if not raw:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Consent request not found"
        )
    if isinstance(raw, bytes):
        raw = raw.decode("utf-8")
    data = json.loads(raw)
    ...
    if str(data.get("provider_id")) != provider.actor_uid:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only requesting provider may claim access",
        )
    if str(data.get("hospital_id")) != str(provider.hospital_id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Consent request belongs to another hospital",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Approved access window is invalid",
        )
    ...
    if datetime.now(timezone.utc) >= access_expires_at:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Approved access window has expired",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Approved device binding is invalid",
        )
    ...
    if result.scalar_one_or_none() is None:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Approving device is no longer active",
        )
...
@router.post(
    ...
    status_code=status.HTTP_200_OK,
    response_model=ConsentCancelResponsePayload,
)
...
@router.get(
    ...
    status_code=status.HTTP_200_OK,
    response_model=ConsentChallengeForPatientPayload,
)
async def get_challenge_for_patient(
    ...
    if data.get("status") != "pending":
        # A pre-audit record is deliberately not a patient-approvable object.
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Challenge expired or not found",
        )
