...
AUDIT_CHAIN_SCOPE = "global"  # default chain_partition for callers that don't specify one
...
_HEAD_LOCK_SQL = text(
    """
    SELECT chain_partition, head_event_id, head_hash, sequence_number, protocol_version, healthy
...
async def _append_once(
    *, actor_uid: str, event_type: str, target_id: str, status: str,
    ...
    async with get_async_engine().begin() as connection:
        ...
        if head_row is not None and not head_row["healthy"]:
...
async def append_audit_log(
    ...
    idempotency_key: str | None = None, chain_partition: str = AUDIT_CHAIN_SCOPE,
) -> bool:
