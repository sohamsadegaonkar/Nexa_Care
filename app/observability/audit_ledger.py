...
def _calculate_hash(payload: dict[str, Any], previous_hash: str) -> str:
    return hashlib.sha256(
        (_canonical_json(payload) + previous_hash).encode("utf-8")
    ).hexdigest()
...
def _is_unique_violation(exc: BaseException) -> bool:
    ...
    return any(
        ...
        and any(
            getattr(candidate, attr, None) == "23505"
...
def _sanitize_metadata(metadata: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    for key, value in metadata.items():
        ...
        if isinstance(value, bool) or value is None or isinstance(value, (int, float)):
            ...
            clean[key] = (
                value if _SAFE_METADATA_VALUE.fullmatch(value) else "[REDACTED]"
            )
            ...
            clean[key] = [
                ...
                if isinstance(item, str) and _SAFE_METADATA_VALUE.fullmatch(item)
                else "[REDACTED]"
                for item in value
            ]
    ...
    *,
    ...
    async with get_async_engine().begin() as connection:
        ...
        head_result = await connection.execute(
            _HEAD_LOCK_SQL, {"chain_partition": chain_partition}
        )
        ...
        row = {
            ...
            "action": event_type,
            "resource": target_id,
            "details": _canonical_json(payload),
...
async def _report_integrity_failure(
    reason: str, trace_id: str, chain_partition: str
) -> None:
    AUDIT_LEDGER_INTEGRITY_FAILURES.labels(
        chain_scope=chain_partition, reason=reason
    ).inc()
    security_logger.critical(
        _canonical_json(
            {
                "event": "audit_ledger_integrity_violation",
                "severity": "critical",
                "chain_scope": chain_partition,
                "reason": reason,
                "trace_id": trace_id,
        ...
        try:
            ...
            if _is_unique_violation(exc) and attempt < _AUDIT_MAX_RETRIES:
                logger.warning(
                    _canonical_json(
                        {
                            "event": "audit_append_concurrency_retry",
                            "trace_id": trace_id,
                            "attempt": attempt,
                            "chain_scope": chain_partition,
                        }
    ...
    *,
    ...
    if not await append_audit_log(
        ...
        raise HTTPException(
            status_code=503,
            detail={
                "error_code": "AUDIT_LEDGER_UNAVAILABLE",
                "message": "Protected operation aborted because audit integrity could not be assured.",
            },
        )
