...
from app.api.v2.patient_record_routes import get_my_access_history, get_patient_audit_trail


@pytest.mark.asyncio
async def test_missing_audit_attributes_remain_null():
    row = {"event_type": "PATIENT_RECORD_READ_SUCCESS", "actor_uid": "provider-id"}
    with patch("app.api.v2.patient_record_routes.read_audit_events", new=AsyncMock(return_value=[row])):
...
async def test_patient_access_history_store_failure_returns_503():
    with patch("app.api.v2.patient_record_routes.read_audit_events", new=AsyncMock(side_effect=RuntimeError())):
