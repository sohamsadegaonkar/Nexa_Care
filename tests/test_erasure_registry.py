...
from app.services.crypto_kms import EncryptionProvider, LocalEnvelopeProvider, PatientDataErased
...
@pytest.mark.parametrize(
    "status", ["requested", "access_blocked", "key_disabled", "deletion_scheduled", "destroyed", "operator_action_required"],
)
...
async def test_cached_dek_still_goes_through_the_gate():
    ...
    with patch.dict(os.environ, {
        ...
        db = _FakeRegistryDB(status="access_blocked")
        with patch.object(EncryptionProvider, "_check_erasure_registry", staticmethod(_REAL_CHECK_ERASURE_REGISTRY)):
