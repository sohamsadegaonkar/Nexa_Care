...
class TestDeviceKeysService:
    ...
    def test_file_exists(self) -> None:
        assert (
            DEVICE_KEYS_PATH.exists()
        ), "nexa-client/packages/app/services/deviceKeys.ts must exist"
    ...
    def test_uses_secure_store(self) -> None:
        code = _read(DEVICE_KEYS_PATH)
        assert (
            "expo-secure-store" in code
        ), "Must use expo-secure-store for private key storage"
        assert "SecureStore" in code, "Must reference SecureStore API"
        assert (
            "setItemAsync" in code
        ), "Must store private key via SecureStore.setItemAsync"
        assert (
            "getItemAsync" in code
        ), "Must read private key via SecureStore.getItemAsync"
    ...
    def test_no_plain_async_storage(self) -> None:
        ...
        assert (
            "AsyncStorage" not in code_no_comments
    ...
    def test_private_key_never_sent_to_backend(self) -> None:
        ...
        assert (
            "device_public_key" in code_no_comments
    ...
    def test_public_key_exported_as_der_base64(self) -> None:
        code = _read(DEVICE_KEYS_PATH)
        assert (
            "wrapEcPublicKeyAsDer" in code or "SubjectPublicKeyInfo" in code
        ), "Must wrap public key in DER X.509 SubjectPublicKeyInfo format"
        assert (
            "base64" in code.lower() or "Base64" in code
...
class TestSecureDeviceScreenIntegration:
    ...
    def test_no_local_keypair_function(self) -> None:
        ...
        assert (
            "function generateDeviceKeypairAlpha" not in code_no_comments
    ...
    def test_has_loading_states(self) -> None:
        ...
        assert (
            "enrolling" in code.lower() or "Registering" in code
    ...
    def test_uses_shared_apiclient(self) -> None:
        ...
        assert "apiClient" in device_keys_code, "Service must use shared apiClient"
    ...
    def test_explains_why_securing_matters(self) -> None:
        ...
        assert (
            "health data" in code.lower() or "protect" in code.lower()
...
class TestDeviceEnrolledScreenIntegration:
    ...
    def test_fetches_device_status(self) -> None:
        ...
        assert (
            "getDevices" in code or "/api/v2/patient/devices" in code
    ...
    def test_shows_device_label(self) -> None:
        code = _read(DEVICE_ENROLLED_PATH)
        assert (
            "device_label" in code or "deviceLabel" in code
    ...
    def test_handles_error_state(self) -> None:
        ...
        assert (
            "error" in code.lower() or "statusError" in code
    ...
    def test_shows_device_fingerprint(self) -> None:
        code = _read(DEVICE_ENROLLED_PATH)
        assert (
            "fingerprint" in code or "device_id" in code
...
class TestEnrollmentFlow:
    ...
    def test_service_generates_keypair_before_enrollment(self) -> None:
        ...
        gen_pos = code.find(
            "generateDeviceKeypair", code.find("generateAndEnrollDevice")
        )
    ...
    def test_enroll_sends_only_public_key(self) -> None:
        ...
        assert (
            "device_public_key" in code_no_comments
        ), "Must define device_public_key in enrollment params"
        ...
        params_section = code_no_comments[
            code_no_comments.find("EnrollDeviceParams") : code_no_comments.find(
                "}", code_no_comments.find("EnrollDeviceParams")
            )
            + 1
        ]
        assert (
            "private" not in params_section.lower()
        ), "EnrollDeviceParams must NOT include any private key field"
        assert (
            "device_public_key" in params_section
    ...
    def test_screen_handles_enrollment_error(self) -> None:
        ...
        assert (
            "enrollment failed" in code.lower() or "try again" in code.lower()
    ...
    def test_screen_resets_step_on_error(self) -> None:
        ...
        assert (
            "setStep('ready')" in code[catch_start:finally_start]
    ...
    def test_delete_key_function_exists(self) -> None:
        ...
        code = _read(DEVICE_KEYS_PATH)
        assert (
            "deleteDeviceKey" in code or "deleteItemAsync" in code
...
class TestCrossFileConsistency:
    ...
    def test_enrollment_endpoint_matches_backend(self) -> None:
        ...
        if backend_path.exists():
            ...
            assert (
                "/api/v2/patient/devices/enroll" in device_keys
            ), "Service enrollment endpoint must match backend route"
            assert (
                "/api/v2/patient/devices" in backend_code
    ...
    def test_enroll_params_match_backend_schema(self) -> None:
        ...
        if backend_path.exists():
            ...
            for field in ["device_public_key", "device_label", "platform"]:
                assert (
                    field in device_keys
                ), f"Service must include {field} in enrollment"
    ...
    def test_devices_list_endpoint_matches_backend(self) -> None:
        ...
        if backend_path.exists():
            assert (
                "/api/v2/patient/devices" in device_keys
...
class TestEnrollmentHappyPathPayload:
    ...
    def test_generate_and_enroll_produces_correct_payload_shape(self) -> None:
        ...
        assert (
            "publicKeyDerBase64" in code
        ), "generateDeviceKeypair must produce publicKeyDerBase64"
        assert (
            "wrapEcPublicKeyAsDer" in code
        ...
        assert (
            "publicKeyDerBase64" in full_flow_body
        ), "generateAndEnrollDevice must use publicKeyDerBase64 from keypair result"

        # ── Step 3: enrollDevice sends correct endpoint ────────────────────
        assert (
            "/api/v2/patient/devices/enroll" in full_flow_body
            or "/api/v2/patient/devices/enroll" in code_no_comments
        ...
        params_section = code_no_comments[
            code_no_comments.find(
                "interface EnrollDeviceParams"
            ) : code_no_comments.find(
        ...
        assert (
            "device_label" in params_section
        ), "EnrollDeviceParams must contain device_label"
        assert "platform" in params_section, "EnrollDeviceParams must contain platform"
        ...
        assert (
            "device_public_key: publicKeyDerBase64" in full_flow_body
