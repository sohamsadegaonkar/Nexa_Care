...
async def test_concurrent_request_blocked(client, mock_redis, mock_db):
    ...
    with (
        patch("app.core.rate_limiter.get_redis_config"),
...
async def test_fail_closed_on_redis_failure(client, mock_db, mock_redis):
    ...
    with (
        ...
        patch(
            "app.services.assurance_service.AssuranceService.create_push_request",
            return_value={"status": "pending", "request_id": "req-1"},
        ),
    ):
        payload = {
            "patient_id": "p1",
...
async def test_release_on_timeout(client, mock_redis, mock_db):
    ...
    with (
        ...
        patch(
            ...
            return_value={"status": "timeout", "patient_id": patient_id},
        ),
    ):
