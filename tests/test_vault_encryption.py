...
async def test_vault_encrypt_decrypt_roundtrip(env_setup, mock_db):
    ...
    # 2. Decrypt
    decrypted_name = await decrypt_vault_field(patient_id, "patient_name", encrypted["patient_name"], mock_db, kms)
...
async def test_auto_migration_on_read(env_setup, mock_db):
    ...
    with patch("app.services.sharding.decrypt_pii_field", return_value="decrypted-plaintext"), \
         patch("app.observability.audit_ledger.append_audit_log", new_callable=AsyncMock) as mock_audit:
        
        # Act
        val = await decrypt_vault_field(patient_id, "patient_name", legacy_token, mock_db, kms)
        
        # Assert
        assert val == "decrypted-plaintext"
        # Verify DB update was triggered
        assert any("UPDATE nexa_vault" in str(call) or "update" in str(call).lower() for call in mock_db.execute.call_args_list)
        # Verify audit
        assert any(call.kwargs.get("event_type") == "PII_ENCRYPTION_MIGRATED" for call in mock_audit.call_args_list)
...
async def test_batch_migration_idempotent(env_setup, mock_db):
    ...
    # Case 1: Legacy row
    row = NexaVault(masked_internal_id=patient_id, patient_name="gAAAAA...", phone=None, aadhaar_abha_id=None)
    
    with patch("scripts.migrate_fernet_to_envelope.decrypt_pii_field", return_value="plain"), \
    ...
    mock_db.reset_mock()
    row_migrated = NexaVault(masked_internal_id=patient_id, patient_name="new-format:1", phone=None, aadhaar_abha_id=None)
