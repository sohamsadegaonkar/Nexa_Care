...
class FakeConsentDB:
    ...
    async def execute(self, stmt):
        ...
        row = (
            ...
            else (self.added[-1] if self.added else None)
        )
...
class TestIssue(unittest.TestCase):
    ...
    @patch(
        "app.services.consent_engine.append_audit_log_or_503", new_callable=AsyncMock
    )
    @patch("app.services.consent_engine.get_consent_redis_client")
    def test_issue_writes_postgres_then_redis_and_audits_in_order(
        ...
        self.assertEqual(
            events,
            [
                "AUDIT_503:CONSENT_GRANT_ATTEMPT",
                "DB_ADD",
                "DB_COMMIT",
                "AUDIT_503:CONSENT_GRANT_SUCCESS",
            ],
        )
    ...
    def test_empty_scope_raises_before_any_audit_or_write(
        self, mock_audit_503, mock_audit
    ):
        db = FakeConsentDB()
    ...
    def test_break_glass_without_reason_code_raises_before_any_write(
        self, mock_audit_503, mock_audit
    ):
        db = FakeConsentDB()
    ...
    @patch("app.services.consent_engine.get_consent_redis_client")
    def test_break_glass_pushes_to_compliance_queue(
        self, mock_get_client, mock_audit_503, mock_audit
    ):
        redis_client = AsyncMock()
        mock_get_client.return_value = redis_client
        ...
        run(
            issue(
                ...
                scope=["clinical"],
                is_break_glass=True,
                reason_code="unconscious patient, ICU",
                assurance_level=AssuranceLevel.BREAK_GLASS,
                assurance_evidence={},
            )
        )
    ...
    def test_postgres_write_failure_rolls_back_and_raises(
        self, mock_audit_503, mock_audit
    ):
        ...
        self.assertEqual(
            mock_audit.await_args.kwargs["event_type"], "CONSENT_GRANT_FAILED"
        )
        self.assertEqual(
            mock_audit.await_args.kwargs["status"], "DURABLE_LOG_WRITE_FAILED"
        )
    ...
    def test_redis_failure_after_postgres_commit_marks_row_revoked_not_a_noop_rollback(
        ...
        self.assertEqual(
            mock_audit.await_args.kwargs["status"], "LIVE_STORE_WRITE_FAILED"
        )
...
class TestValidate(unittest.TestCase):
    ...
    def test_matching_live_token_returns_capability(self, mock_get_client):
        ...
        capability = run(
            validate(token="tok", patient_id="p", clinician_id="c", purpose="routine")
        )
    ...
    def test_missing_token_fails_closed(self, mock_get_client):
        ...
        self.assertIsNone(
            run(
                validate(
                    token="tok", patient_id="p", clinician_id="c", purpose="routine"
                )
            )
        )
    ...
    @patch("app.services.consent_engine.get_consent_redis_client")
    def test_break_glass_payload_without_reason_code_is_rejected_as_malformed(
        ...
        self.assertIsNone(
            run(
                validate(
                    token="tok", patient_id="p", clinician_id="c", purpose="emergency"
                )
            )
        )
...
class TestConsume(unittest.TestCase):
    @patch("app.services.consent_engine.append_audit_log", new_callable=AsyncMock)
    @patch("app.services.consent_engine.get_consent_redis_client")
    def test_consume_deletes_from_redis_and_marks_postgres_row_consumed(
        ...
        capability = run(
            consume(
                db=db, token="tok", patient_id="p", clinician_id="c", purpose="routine"
            )
        )
    ...
    @patch("app.services.consent_engine.append_audit_log", new_callable=AsyncMock)
    @patch("app.services.consent_engine.get_consent_redis_client")
    def test_consume_mismatch_returns_none_without_touching_postgres(
        self, mock_get_client, mock_audit
    ):
        redis_client = AsyncMock()
        redis_client.getdel.return_value = json.dumps(
