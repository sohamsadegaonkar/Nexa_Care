...
from app.core.config import ConfigError, DocumentExtractionConfig, get_document_extraction_config


def remote_config(**overrides):
    values = dict(provider="remote", environment="test", api_url="https://extract.example/v1", api_key="secret", timeout_seconds=1, max_attempts=2)
...
async def test_remote_provider_validates_response_schema():
    response = type("Response", (), {"status_code": 200, "json": lambda self: {
        "patient_name": "Extracted Name", "aadhaar_abha_id": "id", "phone": "phone",
        "diagnoses": [], "lab_results": [], "prescriptions": [], "extraction_confidence": 0.7,
    }})()
    client = AsyncMock()
...
async def test_invalid_upstream_payload_is_terminal():
    response = type("Response", (), {"status_code": 200, "json": lambda self: {"fake": "clinical"}})()
...
async def test_demo_provider_has_no_filename_behavior():
    ...
    results = [await provider.extract_bytes(b"same", mime_type="application/pdf", request_id=name)
...
def test_demo_provider_rejected_in_production_like_environments(monkeypatch, environment):
