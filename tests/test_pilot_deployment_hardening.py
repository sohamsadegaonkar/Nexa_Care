...
def test_migration_script_scopes_url_and_redacts_command_output(
    ...
    def fake_alembic(
        ...
        stdout = (
            f"{run_pilot_migrations.EXPECTED_HEAD} (head)\n"
...
def test_pilot_operations_document_uses_current_contract() -> None:
    ...
    assert "20260905_verification_application" in content
...
def test_governance_contract_names_current_migration_head() -> None:
    ...
    assert "Current head is `20260905_verification_application`" in constitution
    assert "current head `20260905_verification_application`" in security
