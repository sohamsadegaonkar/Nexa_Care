...
def test_migration_script_requires_exact_single_repository_head() -> None:
    assert run_pilot_migrations.repository_heads() == ("20260806_eligibility_reason",)
...
def test_migration_script_scopes_url_and_redacts_command_output(
    ...
    def fake_alembic(
        ...
        stdout = (
            "20260806_eligibility_reason (head)\n" if arguments == ["current"] else ""
        )
...
def test_pilot_operations_document_uses_current_contract() -> None:
    ...
    assert "20260806_eligibility_reason" in content
...
def test_governance_contract_names_current_migration_head() -> None:
    ...
    assert "Current head is `20260806_eligibility_reason`" in constitution
    assert "current head `20260806_eligibility_reason`" in security
