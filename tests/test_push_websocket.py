...
async def test_ws_rejects_without_feature_flag(client):
    with patch("app.api.v2.assurance_routes.PUSH_STATUS_TRANSPORT", "poll"):
        with pytest.raises(Exception): # TestClient raises for closed WS during handshake if not handled
...
async def test_ws_authentication_failure(client):
    with patch("app.api.v2.assurance_routes.PUSH_STATUS_TRANSPORT", "websocket"):
        with patch("app.api.v2.assurance_routes.resolve_provider_session_context", return_value=None):
...
async def test_ws_happy_path_resolution(client, mock_redis):
    ...
    with patch("app.api.v2.assurance_routes.PUSH_STATUS_TRANSPORT", "websocket"), \
         patch("app.api.v2.assurance_routes.resolve_provider_session_context", return_value=mock_provider), \
         patch("app.services.consent_engine.get_consent_redis_client", return_value=mock_redis), \
         patch("app.services.assurance_service.AssuranceService.get_push_status") as mock_get_status:
        ...
        with client.websocket_connect(f"/api/v2/push/{request_id}/ws?token={token}") as websocket:
            # 1. Initial status
...
async def test_ws_initial_resolved_closes_immediately(client, mock_redis):
    ...
    with patch("app.api.v2.assurance_routes.PUSH_STATUS_TRANSPORT", "websocket"), \
         patch("app.api.v2.assurance_routes.resolve_provider_session_context", return_value=mock_provider), \
         patch("app.services.consent_engine.get_consent_redis_client", return_value=mock_redis), \
         patch("app.services.assurance_service.AssuranceService.get_push_status", return_value=mock_status):
