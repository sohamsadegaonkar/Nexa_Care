...
def setup_env(monkeypatch):
    monkeypatch.setenv(
        "KEK_ROOT_SECRET", "test-secret-key-for-kms-v1-at-least-32-chars-long"
    )
...
async def test_full_round_trip(mock_db, mock_redis, kms_provider):
    ...
    encrypted_name = await kms_provider.encrypt_field(
        patient_id, "patient_name", plaintext_name, mock_db
    )
    ...
    encrypted_diag = await kms_provider.encrypt_field(
        patient_id, "diagnoses", plaintext_diagnosis, mock_db
    )

    clinical_row = NexaClinical(
        masked_internal_id=patient_id, diagnoses=encrypted_diag.serialize()
    )
...
async def test_no_consent_no_decrypt(mock_db, mock_redis, kms_provider):
    ...
    with patch(
        "app.services.consent_engine.get_consent_redis_client", return_value=mock_redis
    ):
...
async def test_erased_patient(mock_db, mock_redis, kms_provider):
    ...
    encrypted = await kms_provider.encrypt_field(
        patient_id, "patient_name", "secret", mock_db
    )
    ...
    async def db_execute_side_effect(stmt):
        ...
        if "nexa_vault" in stmt_str:
            res.scalars().first.return_value = NexaVault(
                masked_internal_id=patient_id, patient_name=encrypted.serialize()
            )
            return res
        return res
...
async def test_dek_rotation(mock_db, mock_redis, kms_provider):
    ...
    encrypted_v1 = await kms_provider.encrypt_field(
        patient_id, "patient_name", plaintext, mock_db
    )
    ...
    with (
        ...
        async def db_execute_side_effect(stmt):
            ...
            if "patient_dek_store" in stmt_str:
                ...
                if (
                    "dek_version = :dek_version_1" in stmt_str
                    or "dek_version = 1" in stmt_str
                    or "dek_version = :dek_version" in stmt_str
                ):
                    res.scalar_one_or_none.return_value = dek_v1_row
                else:
            ...
            if "nexa_vault" in stmt_str:
                res.scalars().first.return_value = NexaVault(
                    masked_internal_id=patient_id, patient_name=encrypted_v1.serialize()
                )
                return res
            return res
...
async def test_break_glass_path(mock_db, mock_redis, kms_provider):
    ...
    encrypted_name = await kms_provider.encrypt_field(
        patient_id, "patient_name", "John Doe", mock_db
    )
    encrypted_diag = await kms_provider.encrypt_field(
        patient_id, "diagnoses", "Flu", mock_db
    )
    ...
    async def db_execute_side_effect(stmt):
        ...
        if "nexa_vault" in stmt_str:
            ...
            return res
        if "nexa_clinical" in stmt_str:
            res.scalars().first.return_value = NexaClinical(
