...
def test_job_status_loads_job_before_consent_validation():
    ...
    assert body.index("select(ExtractionJob)") < body.index("validate_consent_for_patient")
    assert "patient_id=str(job.patient_id)" in body


def test_job_status_does_not_read_patient_header_or_query_parameter():
    body = CODE[CODE.index("async def get_extraction_job"):CODE.index("# ── Review queue")]
    assert "X-Patient-Id" not in body
    assert "query_params" not in body


def test_field_review_derives_patient_from_parent_job():
    body = CODE[CODE.index("async def review_extracted_field"):CODE.index("@router.post(\"/fields/{field_id}/approve\"")]
...
def test_nonexistent_field_returns_not_found_without_client_identity_fallback():
    assert 'raise HTTPException(status_code=404, detail="Extracted field not found")' in CODE
...
def test_upload_validates_consent_for_form_patient():
    body = CODE[CODE.index("async def upload_pipeline_document"):CODE.index("@router.get(\"/jobs/{job_id}\"")]
