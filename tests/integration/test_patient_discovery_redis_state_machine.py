...
async def test_real_redis_audit_state_machines_without_postgres(monkeypatch) -> None:
    ...
    try:
        ...
        expired = await issue_handle()
        assert await service.activate_handle(raw_handle=expired.value) is True
        # Expire deterministically without a timing sleep.
        assert await redis.pexpire(_handle_key(expired.value), 0) in (True, 1)
        with pytest.raises(DiscoveryHandleInvalid):
            await service.consume_handle(
                raw_handle=expired.value,
                provider_id="provider-a",
                hospital_id="hospital-a",
                session_binding="session-a",
            )
