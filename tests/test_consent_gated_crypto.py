...
def mock_kms():
    ...
    m.decrypt_field.side_effect = (
        lambda pid,
        name,
        enc,
        db: f"decrypted-{enc.ciphertext.decode() if isinstance(enc.ciphertext, bytes) else enc}"
    )
    return m
...
async def test_consent_gated_decrypt_happy_path(mock_kms, mock_redis, mock_db):
    ...
    mock_vault_row = SimpleNamespace(
        patient_name=mock_val, phone=mock_val, aadhaar_abha_id=None
    )
    mock_vault_res = MagicMock()
    mock_vault_res.scalars().first.return_value = mock_vault_row

    # Mock clinical fetch
    mock_clinical_row = SimpleNamespace(
        diagnoses=mock_val, lab_results=None, prescriptions=None
    )
...
@pytest.mark.asyncio
async def test_consent_gated_decrypt_decrypt_failure_consumes_token(
    mock_kms, mock_redis, mock_db
):
    patient_id = "p1"
    provider_id = "d1"
    token = "tok1"
    ...
    with (
        patch("app.services.consent_engine.validate", return_value=capability),
        patch(
            "app.services.consent_engine.consume", new_callable=AsyncMock
        ) as mock_consume,
        patch(
            "app.services.consent_gated_crypto.append_audit_log_or_503",
            new_callable=AsyncMock,
        ...
        patch(
            "app.services.consent_gated_crypto.append_audit_log", new_callable=AsyncMock
        ) as mock_audit,
    ):
        with pytest.raises(HTTPException) as exc:
            await consent_gated_decrypt(
        ...
        # FAILED audit entry
        assert any(
            call.kwargs.get("event_type") == "CONSENT_GATED_DECRYPT_FAILED"
            for call in mock_audit.call_args_list
        )


@pytest.mark.asyncio
async def test_consent_gated_decrypt_audit_failure_aborts(
    mock_kms, mock_redis, mock_db
):
    capability = ConsentCapability(
        patient_id="p",
...
@pytest.mark.asyncio
async def test_consent_gated_decrypt_passes_hospital_binding(
    mock_kms, mock_redis, mock_db
):
    capability = ConsentCapability(
    ...
    with patch(
        "app.services.consent_engine.validate",
        new_callable=AsyncMock,
