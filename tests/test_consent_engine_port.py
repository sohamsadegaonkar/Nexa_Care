...
async def test_issue_routine_happy_path():
    ...
    with (
        patch(
            "app.services.consent_engine.issue", new_callable=AsyncMock
        ) as mock_issue,
        patch(
        ...
        patch(
            "app.services.consent_engine.RedisAssuranceVerifier", new_callable=MagicMock
...
async def test_issue_break_glass_happy_path():
    ...
    with (
        ...
        assert (
            mock_issue.call_args.kwargs["assurance_level"] == AssuranceLevel.BREAK_GLASS
        )
...
async def test_issue_break_glass_missing_reason_code_rejects():
    db = AsyncMock(spec=AsyncSession)
    with pytest.raises(
        ValueError, match="Break-glass grants require a non-empty reason_code"
    ):
        await issue_break_glass(
            patient_id="p1", clinician_id="c1", reason_code="", db=db
        )
