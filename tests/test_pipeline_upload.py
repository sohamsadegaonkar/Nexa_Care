...
from app.api.v2.pipeline_routes import (
    CommitJobRequest,
    _validated_upload_type,
    commit_extraction_job,
)
...
def test_upload_validation_uses_magic_mime_and_sanitizes_filename():
    name, mime = _validated_upload_type(
        "../../panel.pdf", "application/pdf", b"%PDF-1.7\n"
    )
...
def test_ordinary_filenames_do_not_change_validation(name):
    assert (
        _validated_upload_type(name, "application/pdf", b"%PDF-1.7")[1]
        == "application/pdf"
    )
...
async def test_local_storage_roundtrip_enforces_patient_tenant_ownership(tmp_path):
    ...
    assert (
        await storage.get_document_bytes(
            stored.storage_ref, tenant_id="tenant-a", patient_id="patient-a"
        )
        == b"%PDF-private"
    )
    with pytest.raises(DocumentStorageError):
        await storage.get_document_bytes(
            stored.storage_ref, tenant_id="tenant-a", patient_id="patient-b"
        )
...
def test_ocr_identity_is_not_a_candidate_and_no_default_field_is_created():
    document = ExtractedMedicalDocument(
        patient_name="OCR Name",
        aadhaar_abha_id="ocr-id",
        phone="ocr-phone",
        diagnoses=[],
        lab_results=[],
        prescriptions=[],
...
async def test_malformed_job_id_is_not_uuid5_coerced():
    ...
    assert result == {
        "status": "extraction_failed_terminal",
