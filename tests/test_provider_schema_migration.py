...
from app.models.provider import HospitalRegistry, ProviderCredential, ProviderHospitalAffiliation, ProviderIdentity
...
def _source() -> str:
    return (ROOT / "alembic" / "versions" / "20260714_add_provider_schema.py").read_text(encoding="utf-8")
...
def test_provider_revision_precedes_patient_auth_identity_head() -> None:
    ...
    assert _scripts().get_heads() == ["20260717_secure_document_pipeline"]
...
def test_foreign_key_targets_precede_referencing_tables() -> None:
    source = _source()
    assert source.index('has_table("hospital_registry")') < source.index('has_table("provider_identity")')
    assert source.index('has_table("provider_identity")') < source.index('has_table("provider_hospital_affiliation")')
    assert source.index('has_table("provider_identity")') < source.index('has_table("provider_credential")')
...
def test_existing_database_upgrade_is_guarded_and_downgrade_is_non_destructive() -> None:
    source = _source()
