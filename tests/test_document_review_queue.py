...
class TestReviewRoutes(unittest.TestCase):
    ...
    def test_approve_owned_pending_review_writes_shards_and_marks_approved(self, mock_split, mock_audit):
        ...
        # select owned review + two shard inserts. KMS itself is mocked here
        # so this route test does not depend on KEK_ROOT_SECRET or real DEK
        # lookup behavior.
        self.assertEqual(len(db.executions), 3)
    ...
    def test_approve_rolls_back_on_db_error(self, mock_split, mock_audit_503, mock_audit):
        ...
        # DB_COMMIT after ATTEMPT mirrors generate_dek() persisting a DEK
        # before the vault/clinical shard inserts, without exercising real
        # KMS config in this rollback test.
        self.assertEqual(
            events,
            [
