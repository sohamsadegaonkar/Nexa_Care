...
APPROVAL_ROOTS = [
    ROOT / "nexa-client/packages/app/features/approval",
    ROOT / "nexa-client/packages/app/features/patient",
    ROOT / "nexa-client/apps/next/app/push-approval",
    ROOT / "nexa-client/apps/expo/app/push-approval",
]
FIELDS = [
    "request_id",
    "patient_id",
...
def test_frontend_and_backend_share_versioned_canonical_json_contract():
    frontend = DEVICE_KEYS.read_text(encoding="utf-8")
    backend = VERIFIER.read_text(encoding="utf-8")
    assert all(f"params.{field}" in frontend for field in FIELDS)
    assert all(f'"{field}"' in backend for field in FIELDS)
    assert "JSON.stringify" in frontend
    assert "sort_keys=True" in backend and 'separators=(",", ":")' in backend
    assert "nexa-consent-v2" in frontend and "nexa-consent-v2" in backend
    assert '.join("|")' not in frontend and "signing_input_9" not in backend
...
def test_reachable_patient_approval_has_no_legacy_responder():
    ...
    for root in APPROVAL_ROOTS:
        ...
        for path in root.rglob("*"):
            ...
            if (
                "/respond" in code
                or "respondToPushRequest" in code
                or "getPushRequestStatus" in code
            ):
