...
def provider_context(
    ...
    hospital_id: str = "00000000-0000-0000-0000-000000000002",
) -> ProviderContext:
    ...
    with patch(
        ...
        assert (
            await validate(
                ...
                provider_id="provider-1",
                hospital_id="hospital-1",
                requested_category="timeline_view",
            )
            is not None
        )
        for kwargs in (
...
async def test_foreign_provider_or_hospital_cannot_claim():
    ...
    request = approved_request(
        provider_id=owner.actor_uid, hospital_id=str(owner.hospital_id)
    )
    seed_request(redis, request)
    foreign_provider = provider_context("00000000-0000-0000-0000-000000000003")
    foreign_hospital = provider_context(
        hospital_id="00000000-0000-0000-0000-000000000004"
    )
    with patch("app.api.v2.consent_routes.get_redis_client", return_value=redis):
        for provider in (foreign_provider, foreign_hospital):
            with pytest.raises(HTTPException) as exc:
                await claim_approved_access(
                    "request-1", Response(), provider, MagicMock()
                )
