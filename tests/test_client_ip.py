...
def _request(peer: str | None, forwarded: str | None = None) -> Request:
    ...
    scope = {"type": "http", "method": "GET", "path": "/", "headers": headers,
             "client": (peer, 1234) if peer is not None else None, "server": ("test", 80),
...
def test_spoofed_header_from_untrusted_peer_is_ignored() -> None:
    assert resolve_client_ip(_request("203.0.113.7", "192.0.2.9"), trusted_proxy_networks("10.0.0.0/8")) == "203.0.113.7"


def test_one_trusted_proxy_returns_forwarded_client() -> None:
    networks = trusted_proxy_networks("10.0.0.0/8")
    assert resolve_client_ip(_request("10.0.0.5", "203.0.113.7"), networks) == "203.0.113.7"
...
def test_ipv6_chain_is_supported() -> None:
    ...
    assert resolve_client_ip(_request("2001:db8:1::5", "2001:db8:2::9"), networks) == "2001:db8:2::9"
