...
def mock_provider():
    ...
    return ProviderContext(
        provider=ProviderIdentityContext(
            provider_id=pid, display_name="Dr. Reg", contact_email="r@ex.com"
        ),
...
def client(fake_async_redis, mock_provider):
    ...
    m_supabase.table.return_value.select.return_value.order.return_value.limit.return_value.execute.return_value.data = [
        {"record_hash": "GENESIS"}
    ]
...
async def test_v1_lifecycle(client):
    ...
    m_supabase = MagicMock()
    m_supabase.table.return_value.select.return_value.eq.return_value.limit.return_value.execute.return_value.data = [
        {"masked_internal_id": p_id, "diagnoses": ["H"]}
    ]
    ...
    with patch("app.api.routes.get_supabase_client", return_value=m_supabase):
        assert (
            test_client.get(
                "/view-record/clinical", headers={"X-Consent-Token": token}
            ).status_code
            == 200
        )
        assert (
