...
class TestVerifyBiometricBinding(unittest.TestCase):
    ...
    def test_matching_unrevoked_binding_returns_true(self, mock_get_client, mock_compute):
        mock_compute.return_value = "abc123hash"
        client, _ = make_fake_select_client(
            FakeResult(error=None, data={"bio_verifier_hash": "abc123hash", "revoked_at": None})
        )
        mock_get_client.return_value = client

        result = run(verify_biometric_binding("NFC-001", "pulse-data", "patient-uuid-123"))
    ...
    @patch("app.services.biometric_registry.compute_bio_verifier")
    @patch("app.services.biometric_registry.get_supabase_client")
    def test_revoked_binding_returns_false_even_if_verifier_matches(self, mock_get_client, mock_compute):
        mock_compute.return_value = "abc123hash"
        client, _ = make_fake_select_client(
            FakeResult(error=None, data={"bio_verifier_hash": "abc123hash", "revoked_at": "2026-01-01T00:00:00Z"})
        )
        mock_get_client.return_value = client
    ...
    def test_db_error_returns_false(self, mock_get_client):
        client, _ = make_fake_select_client(FakeResult(error="connection reset", data=None))
        mock_get_client.return_value = client
...
class TestEnrollBiometricBindingWithAudit(unittest.TestCase):
    ...
    def test_success_logs_attempt_then_success_in_order(self, mock_audit, mock_enroll):
        ...
        self.assertEqual(first_call.kwargs["event_type"], "BIOMETRIC_ENROLLMENT_ATTEMPT")
        self.assertEqual(second_call.kwargs["event_type"], "BIOMETRIC_ENROLLMENT_SUCCESS")

    @patch("app.services.biometric_registry.enroll_biometric_binding")
    @patch("app.services.biometric_registry.append_audit_log_or_503")
    def test_db_write_failure_logs_failed_event_then_raises_502(self, mock_audit, mock_enroll):
        ...
        first_call, second_call = mock_audit.call_args_list
        self.assertEqual(first_call.kwargs["event_type"], "BIOMETRIC_ENROLLMENT_ATTEMPT")
        self.assertEqual(second_call.kwargs["event_type"], "BIOMETRIC_ENROLLMENT_FAILED")

    @patch("app.services.biometric_registry.enroll_biometric_binding")
    @patch("app.services.biometric_registry.append_audit_log_or_503")
    def test_audit_write_failure_on_attempt_aborts_before_any_db_write(self, mock_audit, mock_enroll):
    ...
    @patch("app.services.biometric_registry.enroll_biometric_binding")
    @patch("app.services.biometric_registry.append_audit_log_or_503")
    def test_audit_write_failure_on_success_log_still_raises_503(self, mock_audit, mock_enroll):
        ...
        mock_enroll.return_value = True
        mock_audit.side_effect = [None, HTTPException(status_code=503, detail="audit down")]
