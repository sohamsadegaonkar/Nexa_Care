...
async def test_revoke_break_glass_happy_path(client, mock_provider):
    ...
    with patch("app.api.v2.consent_routes.append_audit_log_or_503", new_callable=AsyncMock) as mock_audit, \
         patch("app.api.v2.consent_routes.consent_engine.revoke", new_callable=AsyncMock) as mock_revoke:
...
async def test_revoke_non_break_glass_token_returns_400(client, mock_provider):
    ...
    with patch("app.api.v2.consent_routes.append_audit_log_or_503", new_callable=AsyncMock):
        response = client.post(
            "/api/v2/consent/break-glass/revoke",
