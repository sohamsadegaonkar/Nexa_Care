...
| SEC-014 | Document authenticity | Unavailable/malformed extraction could fabricate clinical values or lose evidence. | No fabricated fallback or production-like mock; malformed or incomplete output fails closed; identity conflict blocks commit; document confidence never substitutes for field confidence; missing page, bounding box, source text, provider/model, and verifier evidence remain explicitly unavailable. Textract Queries are controlled extraction-only prompts, and only authentic QUERY_RESULT confidence/geometry/source text may use provider-field provenance. Candidate PHI is patient-DEK encrypted and bound to tenant, patient, provider, source document, and job before authorized display. | `extractor.py`; `field_evidence.py`; `extraction_evidence_adapter.py`; `pipeline_orchestrator.py`; `pipeline_routes.py`; `document_processing_gate.py`; `20260727_document_processing_bindings.py`; `20260801_textract_candidates.py` | adversarial field-evidence tests; Textract mapping/error/log tests; candidate cross-patient/provider isolation; document extractor/pipeline/server-side consent tests | Adapter and automated provider-response validation enforced locally; live AWS proof pending | 2026-07-30 |
