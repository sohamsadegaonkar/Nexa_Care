...
| SEC-051 | Durable provider verification scheduling and registry worker boundary | Uncoordinated or unconstrained registry scheduling could flood external sources, retry indefinitely, leak SSRF requests to internal/cloud metadata networks, conflate network transport retry with observation evidence, mutate lifecycle states without durable work records, allow multiple workers to concurrently process duplicate tasks, lose crash recovery state, or bypass human review on open review queues. | (1) Permanent authority invariant: REGISTRY SCHEDULER != DURABLE VERIFICATION WORK != NETWORK ATTEMPT != REGISTRY ADAPTER != LOOKUP INVOCATION != REGISTRY OBSERVATION != DECISION POLICY != SYSTEM EXECUTION AUTHORITY != LIFECYCLE MUTATION != CLINICAL AUTHORITY. (2) Durable work queue table (`provider_verification_work`) with XOR target constraint, status enum, attempt bounds, and partial unique indexes on active work (`PENDING`, `CLAIMED`). (3) Invocation lineage is frozen for a work row: `provider_verification_work.id == RegistryLookupInvocation.invocation_id` and `provider_verification_work.created_at == RegistryLookupInvocation.invoked_at` across claims, retries, lease reclaims, exhaustion, and Phase 5E application. (4) Worker leasing uses `SELECT ... FOR UPDATE SKIP LOCKED` with reclaimable `lease_expires_at`; a claim does not increment `attempt_count`, which is incremented only immediately before outbound network attempt; ownership and database-current lease time are revalidated before Phase 5E application. (5) Strict separation of transport retry vs observation emission: transient retryable outages reschedule with backoff and emit ZERO evidence until max attempts exhausted; exhausted attempts emit one normalized `SOURCE_UNAVAILABLE` observation to Phase 5E without replacing the frozen invocation facts. (6) Scheduled-due lifecycle transition, work insertion, and scheduler audit/outbox events commit or roll back atomically. Phase 5E observation application and work terminalization likewise share one outer transaction; a failure before its commit rolls back evidence, lifecycle, audit/outbox, idempotency, and work-terminal mutations together. (7) Open review queue integrity: any `OPEN` review work blocks automatic scheduling. (8) Network safety: HTTPS-only, no URL credentials, host allowlist, strict SSRF blocking (loopback, private RFC 1918, link-local, multicast, cloud metadata), 1 MiB response size limit, redirect rejection, JSON content-type enforcement, `follow_redirects=False`, `trust_env=False`, and no raw body or Authorization logging. (9) Source cadence: `SourceAutomationPolicy` `recheck_interval_seconds` enforced as positive int; Phase 5E computes `next_review_at` on positive complete recheck. (10) Zero new HTTP routes (frozen at 26); single Alembic head `20260906_verification_scheduler`. | `app/models/provider.py`; `app/services/provider_verification_network.py`; `app/services/provider_verification_scheduler.py`; `app/services/provider_verification_worker.py`; `alembic/versions/20260906_verification_scheduler.py` | `tests/test_provider_verification_network.py`; `tests/test_provider_verification_scheduler.py`; `tests/test_provider_verification_worker.py`; `tests/integration/test_provider_verification_scheduler_postgres.py` | Enforced locally on disposable PostgreSQL 16; zero new HTTP routes; live external network integrations deferred | 2026-09-06 |
