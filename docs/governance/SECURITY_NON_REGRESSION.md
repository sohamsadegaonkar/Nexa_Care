...
| SEC-036 | Clinical conflict and source-provenance integrity | Provider-controlled fact identifiers, concurrent graph mutation, stale consent/erasure state, mutable provenance or mismatched member identifiers could fabricate conflict authority, create source cycles, cross patients/tenants, or leak raw database errors. | Only private Nexa parser context may create exact fact identity; insufficient context remains explicit ambiguity. PostgreSQL transaction locks serialize source graphs per tenant/patient, conflict graphs per tenant/patient/job/fact, and adjudication mutations per case. Source-edge mutation reloads both documents, proves provider/workflow binding, revalidates current capability and erasure, and rejects cycles/depth overflow. Composite member/candidate evidence integrity and append-only triggers protect provenance. Prior evidence requires one exact durable candidate/decision/job graph. Successful A1 changes audit value-free in the same transaction; AUTO_COMMIT remains disabled. | `clinical_evidence_integrity.py`; `pipeline_orchestrator.py`; `adjudication.py`; `pipeline.py`; `20260814_conflict_supersession.py` | A1R unit, production orchestrator/upload, concurrent PostgreSQL graph/conflict/adjudication, immutability, migration, audit and full-loop tests | Enforced locally. Fresh upgrade, previous-head downgrade/re-upgrade, old-row preservation, append-only constraint probes, concurrent PostgreSQL qualification, real PostgreSQL/Redis full loop, and full backend passed on disposable infrastructure on 2026-08-14. Deployment and production qualification remain required. | 2026-08-14 |
