...
| SEC-041 | Provider clinical trust | Account login, a legacy clinician role, a registration-number claim, or MFA enrollment could be mistaken for current clinical authority. | Clinical authority is the conjunction of independent active account/credential, explicit contact policy, current professional verification, verified facility, current active affiliation, fixed server-owned capability and required session/MFA assurance. Legacy state never grants trust. Delegated steps reload current state and are valid for one bounded operation for at most 60 seconds. Trust-store failure, contradictory/naive trust data, and unresolved contact policy fail closed. Consent remains a distinct, later patient authorization and cannot override invalid clinical trust. | `provider.py`; `provider_capabilities.py`; `clinical_eligibility.py`; `20260830_provider_trust.py` | clinical eligibility, capability, trust-model, migration, audit registry, provider auth/MFA and consent regression tests | Trust primitives only; no patient-facing route is rewired in this slice. Disposable PostgreSQL migration qualification remains required. | 2026-08-30 |

### Provider clinical-trust boundary (Slice 1)
...
`ClinicalEligibilityService` is deliberately independent of existing NFC,
consent, record, pipeline, and emergency routes in this slice. Interactive
evaluation requires an active bound provider session and MFA evidence; Basic
authentication is not clinical-session assurance. `emergency.attempt` uses the
existing break-glass recent-MFA policy. Delegated work stores only non-secret
initiation provenance, reloads current trust at every protected checkpoint, and
may authorize one bounded step for no more than 60 seconds. Logout does not
erase a valid delegated initiation, while current account disable, credential
disable, professional suspension, facility suspension, affiliation revocation,
or capability removal denies the next check.

The exact contact-assurance rule, professional-grace duration, retention, and
external NMC/SMC/ABDM integration remain policy decisions. An absent contact
policy is a fail-closed denial; grace has no effect unless a separately
approved expiration is recorded. This documentation makes no DPDP, NMC, ABDM,
HPR/HFR, Aadhaar, or production-integration compliance claim.
