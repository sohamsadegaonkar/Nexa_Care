...
function primaryRole(roles: string[]): string {
  return (
    ['admin', 'privacy_officer', 'auditor', 'clinician', 'receptionist'].find((role) =>
      roles.includes(role)
    ) ??
    roles[0] ??
    'none'
    ...
    try {
      ...
      const next: ProviderSession = {
        ...
        hospital: {
          ...
          facility_code: '',
          display_name: data.hospital_name,
        },
      }
      setSession(next)
  ...
  const login = useCallback(
    (email: string, password: string): Promise<LoginResult> => {
      ...
      const operation = (async () => {
        ...
        setLoginError(null)
        try {
          const result = await NexaApiClient.providerWebLogin({
            login_identifier: email.trim(),
            password,
          })
          if (result.status === 'mfa_required') {
            setMfaDetail('Multi-factor authentication required.')
