...
describe('RequestConsentScreen web consent flow', () => {
  ...
  it('renders DOM selects without viewport prop warnings and updates all values', () => {
    ...
    expect(screen.getAllByRole('option').map((option) => option.textContent)).toEqual(expect.arrayContaining([
      'Treatment', 'Diagnostic Review', 'Clinical', 'Full Record', '5 minutes', '30 minutes',
    ]))
  ...
  it('submits the controlled contract once with the authenticated hospital context', async () => {
    ...
    const pending = new Promise<{ request_id: string }>((resolve) => { resolveRequest = resolve })
    const requestConsent = vi.spyOn(NexaApiClient, 'requestConsent').mockReturnValue(
      pending as ReturnType<typeof NexaApiClient.requestConsent>,
    )
    renderWithTamagui(<RequestConsentScreen />)
    ...
    await waitFor(() => expect(push).toHaveBeenCalledWith(
      '/doctor/waiting?request_id=request-1&patient_id=patient-1',
    ))
  })
