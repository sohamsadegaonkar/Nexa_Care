...
function ProvenanceBadge({
  ...
  if (isAI) {
    ...
    const color =
      pct != null ? (pct >= 90 ? '$yellow10' : pct >= 70 ? '$orange10' : '$red10') : '$blue10'
...
export function PatientRecordViewerScreen() {
  ...
  const consentValidation: ConsentValidation | null = useMemo(
    () =>
      accessGrant
        ? {
            valid: true,
  ...
  const fetchRecord = useCallback(async () => {
    ...
    if (!patientId || !accessGrant || accessGrant.patientId !== patientId || !hospitalId) {
      setError(
        'Approved access is no longer available. Return to consent history and securely reclaim access.'
      )
    ...
    try {
      ...
      const mapped: PatientSummary = {
        ...
        clinical_summary: {
          ...
          active_medications:
            data.clinical_summary?.active_medications ?? (data as any).medications ?? [],
      ...
      if (terminalAccessFailure) {
        clearAccessGrant()
        setError('Approved access is no longer valid. Request or reclaim access again.')
      } else {
        setError(
          'Secure record validation was interrupted. The capability was not discarded; retry validation.'
        )
      }
  ...
  useEffect(() => {
    ...
    const remaining = Math.max(
      0,
      Math.floor((Date.parse(accessGrant.expiresAt) - Date.now()) / 1000)
    )
    ...
    return (
      ...
      >
        ...
        <Paragraph
          ...
          color="$color11"
        >
          You must be logged in to view patient records.
        </Paragraph>
        <Button
          theme="blue"
          size="$4"
    ...
    return (
      ...
      >
        ...
        <Paragraph
          ...
          fontSize={11}
        >
          The frontend timer is a UX indicator only. All data access is independently validated
  ...
  const medications =
    summary.clinical_summary?.active_medications ??
    summary.clinical_summary?.current_medications ??
    []
  const labs = summary.clinical_summary?.recent_labs ?? []
  const conditions =
    summary.clinical_summary?.chronic_conditions ??
    summary.clinical_summary?.active_conditions ??
    []

  const allergiesInScope =
    !consentValidation?.scope ||
      ...
      >
        ...
        {activeTab === 'vitals' && (
          <YStack gap="$3">
            ...
            {vitals.length === 0 ? (
              <Paragraph color="$color11">No vitals recorded.</Paragraph>
            ) : (
              vitals.map((v, i) => (
                <XStack
                  key={i}
        ...
        {activeTab === 'prescriptions' && (
          <YStack gap="$3">
            ...
            {medications.length === 0 ? (
              <Paragraph color="$color11">No active medications.</Paragraph>
            ) : (
              medications.map((m, i) => (
                <XStack
                  key={i}
        ...
        {activeTab === 'labs' && (
          <YStack gap="$3">
            ...
            {labs.length === 0 ? (
              <Paragraph color="$color11">No lab results available.</Paragraph>
            ) : (
              labs.map((l, i) => (
                <YStack
                  key={i}
                ...
                >
                  ...
                  >
                    ...
                    <Text
                      color={l.is_abnormal ? '$red10' : '$color12'}
                      fontWeight="800"
                    >
                      {l.value} {l.unit}
                    </Text>
          ...
          <YStack gap="$3">
            ...
            >
              ...
              <Text
                ...
                fontSize={13}
              >
                These allergies must be reviewed before any treatment decision. AI-extracted allergy
                values must be verified by a clinician before use in treatment decisions.
              </Text>
            </Card>
            {allergies.length === 0 ? (
              <Paragraph color="$color11">No known allergies.</Paragraph>
            ) : (
              <XStack
                flexWrap="wrap"
                gap="$2"
        ...
        {activeTab === 'timeline' && (
          <YStack gap="$3">
            ...
            {timeline.length === 0 ? (
              <Paragraph color="$color11">No timeline events available.</Paragraph>
            ) : (
              timeline.map((t) => (
                <YStack
                  key={t.event_id}
        ...
        {activeTab === 'access' && (
          <YStack gap="$4">
              ...
              {secondsRemaining !== null && (
                <>
                  ...
                  >
                    ...
                    <Text
                      color={secondsRemaining < 60 ? '$red10' : '$color12'}
                      fontWeight="700"
                    >
                      {formatCountdown(secondsRemaining)}
                    </Text>
            ...
            <Paragraph
              ...
              fontSize={12}
            >
              All data access is consent-gated and audited. This access will be recorded and may
              trigger patient and compliance notifications.
            </Paragraph>
          </YStack>
        )}
      </YStack>
