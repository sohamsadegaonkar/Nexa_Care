...
export function PatientApprovalScreen({ requestId }: PatientApprovalScreenProps) {
  ...
  useEffect(() => {
    ...
    void fetchChallenge(requestId)
      ...
      .catch(() => {
        if (!cancelled) {
          setError('This request has expired or is unavailable.')
          setState('error')
        }
      })
    return () => {
  ...
  if (state === 'loading' || state === 'submitting')
    return (
      ...
      >
        <Spinner />
        <Paragraph>
          {state === 'loading' ? 'Loading request...' : 'Submitting signed decision...'}
        </Paragraph>
      </YStack>
    )
  if (state === 'approved')
    return (
      ...
      >
        <H4>Approved</H4>
        <Paragraph>Signed consent approval verified.</Paragraph>
      </YStack>
    )
  if (state === 'denied')
    return (
      ...
      >
        <H4>Denied</H4>
        <Paragraph>Signed denial recorded.</Paragraph>
      </YStack>
    )
  if (state === 'expired')
    return (
  ...
  return (
    ...
    >
      ...
      <Paragraph>
        {challenge?.provider_name} requests {challenge?.scope} access for {challenge?.purpose}.
      </Paragraph>
      <Button
        theme="green"
        onPress={approve}
