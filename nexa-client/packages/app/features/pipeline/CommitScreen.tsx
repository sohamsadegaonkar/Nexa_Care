...
export function CommitScreen() {
  ...
  const fieldStats = useMemo(() => {
    ...
    const autoApproved = job.extracted_fields.filter((f) => f.status === 'auto_approved')
    const humanApproved = job.extracted_fields.filter((f) => f.status === 'approved')
    const edited = job.extracted_fields.filter((f) => f.status === 'edited')
    const rejectedFields = job.extracted_fields.filter((f) => f.status === 'rejected')
    const unresolvedFields = job.extracted_fields.filter((f) => f.status === 'needs_review')
  ...
  const canCommit =
    fieldStats.needsReview === 0 && fieldStats.committable > 0 && commitState === 'idle'
      ...
      >
        ...
        >
          <Text
            ...
            fontWeight="600"
          >
            {commitResult.committed_fields_count} field
            {commitResult.committed_fields_count !== 1 ? 's' : ''} committed to patient record
          </Text>
          <Separator />
          <YStack gap="$2">
            <Text
      ...
      {fieldStats.hasHighOrCriticalRisk && (
        ...
        >
          ...
          <Paragraph
            ...
            fontSize="$3"
          >
            This job contains fields flagged as HIGH or CRITICAL risk. Please double-check these
            fields before committing to the patient record. High-risk AI extractions may contain
            errors.
          </Paragraph>
        </Card>
      )}
      ...
      {fieldStats.needsReview > 0 && (
        ...
        >
          <Text
            ...
            fontWeight="600"
          >
            ⚠ {fieldStats.needsReview} field{fieldStats.needsReview !== 1 ? 's' : ''} still need
            review before you can commit
          </Text>
          <Paragraph
            color="$orange10"
            fontSize="$3"
