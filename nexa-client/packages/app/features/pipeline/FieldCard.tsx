...
import { NexaApiClient, type ExtractedField, type FieldReviewResponse, ApiError } from '../../utils/apiClient'
...
export function FieldCard({ field, consentToken, onFieldUpdated, onSourcePageClick }: FieldCardProps) {
  ...
  const isAdjudicated = field.status === 'approved' || field.status === 'edited' || field.status === 'rejected'
        ...
        <YStack gap="$2" bg="$blue2" br="$3" p="$3">
          ...
          <XStack gap="$2">
            ...
            <Button
              size="$2"
              chromeless
              onPress={() => { setEditMode(false); setEditValue(field.corrected_value ?? field.raw_value); }}
            >
              Cancel
            </Button>
          </XStack>
        ...
        <YStack gap="$2" bg="$red2" br="$3" p="$3">
          ...
          <XStack gap="$2">
            ...
            <Button
              size="$2"
              chromeless
              onPress={() => { setRejectMode(false); setRejectNotes(''); }}
            >
              Cancel
            </Button>
          </XStack>
