...
export function PatientTransparencyScreen({ patientUuid }: Props) {
  ...
  useEffect(() => {
    const fetchAccessLog = async () => {
      try {
        const res = await fetch(`http://localhost:8000/api/v2/patient/${patientUuid}/access-log`)
  ...
  return (
    <ScrollView flex={1} bg="$background" p="$5">
      <YStack gap="$4">
        <Text fontSize={24} fontWeight="900" color="$color12">Who Accessed My Data</Text>
        ...
        {loading ? (
          ...
          logs.map((log, index) => (
            <Card key={index} p="$4" bg="$color2" borderWidth={1} borderColor="$borderColor">
              <YStack gap="$2">
                <Text fontSize={15} color="$color11">{log.timestamp}</Text>
                <Text fontSize={17} fontWeight="800" color="$color12">
                  {log.clinician} • {log.hospital}
                </Text>
                <Text color="$blue11">{log.purpose}</Text>
                <Text fontSize={13} color="$green10">Assurance: {log.assurance}</Text>
              </YStack>
            </Card>
          ))
        )}
