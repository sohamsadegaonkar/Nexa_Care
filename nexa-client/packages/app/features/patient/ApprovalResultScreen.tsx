...
export default function ApprovalResultScreen() {
  ...
  const handleRevoke = async () => {
    ...
    try {
      await apiClient.delete(
        `/api/v2/consent/grants/${params.requestId}` as `${string}`,
      )
      router.replace('/patient/access-history')
    } catch {
      // Silently fail — user can try again from access history
    } finally {
      setRevoking(false)
    }
  }
  ...
  if (isExpired) {
    return (
      <YStack f={1} bg="$background" p="$4" gap="$4" jc="center" ai="center">
        ...
        <Button theme="blue" size="$4" mt="$2" onPress={() => router.replace('/patient/access-history')}>
  ...
  if (isApproved) {
    return (
      <YStack f={1} bg="$background" p="$4" gap="$4" jc="center" ai="center">
        ...
        <H2 col="$green10" ta="center">Access Granted</H2>

        <Paragraph col="$colorSubdued" ta="center" size="$4" mw={320}>
          Access granted to {providerName}. The doctor can now view the
          approved data until access expires.
        </Paragraph>

        <YStack bg="$backgroundHover" br="$4" p="$4" gap="$3" w="100%" mw={360}>
          <YStack>
            <Paragraph col="$colorSubdued" size="$2" textTransform="uppercase" letterSpacing={1}>
              Provider
            </Paragraph>
            <Text col="$color" size="$4" fontWeight="600">{providerName}</Text>
          </YStack>
          ...
          <XStack jc="space-between" ai="center">
            ...
            <Text col="$orange10" size="$5" fontWeight="700" fontFamily="$mono">
              {remaining || '—'}
            </Text>
          </XStack>
        </YStack>

        <YStack gap="$3" w="100%" mt="$2">
  ...
  return (
    <YStack f={1} bg="$background" p="$4" gap="$4" jc="center" ai="center">
      ...
      <H2 col="$red10" ta="center">Access Denied</H2>
