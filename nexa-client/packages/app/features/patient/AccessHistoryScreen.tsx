  ...
  return (
    <YStack f={1} bg="$background">
      <YStack px="$4" pt="$4" pb="$2">
        <H2 col="$color" size="$7">Access History</H2>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, gap: 8 }}>
        ...
        {loading && (
          <YStack ai="center" py="$8" gap="$3">
            ...
            <Paragraph col="$colorSubdued" size="$4">Loading history...</Paragraph>
          </YStack>
        )}
        ...
        {!loading && !error && history.map((entry) => (
          <YStack key={entry.audit_id} bg="$backgroundHover" br="$4" p="$3" gap="$2">
            ...
            {/* Doctor + hospital */}
            <XStack ai="center" gap="$2" fw="wrap">
              <Paragraph col="$color" size="$3" fontWeight="600">{entry.doctor_name}</Paragraph>
              <Text col="$colorSubdued" size="$2" o={0.4}>•</Text>
              <Paragraph col="$colorSubdued" size="$3">{entry.hospital_name}</Paragraph>
            </XStack>

            {/* Purpose */}
            <Paragraph col="$colorSubdued" size="$2" o={0.8}>
              ...
              <XStack fw="wrap" gap="$1" mt="$1">
                {entry.data_categories.map((cat) => (
                  <YStack key={cat} bg="$backgroundFocus" br="$2" px="$2" py="$1">
                    <Text col="$colorSubdued" size="$1">{cat}</Text>
                  </YStack>
                ))}
              </XStack>
            )}
      ...
      <YStack p="$4" gap="$3" bc="$background">
        <Separator />
        <Button f={1} theme="blue" size="$3" onPress={() => router.push('/patient/timeline')}>
