...
export async function updatePatientPolicy(patientUuid: string, policy: string): Promise<PatientPolicyResponse> {
  const res = await apiClient.put<PatientPolicyResponse>(`/api/v2/patient/${patientUuid}/policy`, {
